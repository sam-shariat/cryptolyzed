...
const txs_query =({days,selectedChains})=> 
{
    ...
    `(${chains[chn].name.toLowerCase()}.blocks) AS ${chains[chn].name.toLowerCase()}_blocks_per_day,
    (${chains[chn].name.toLowerCase()}.txs) AS ${chains[chn].name.toLowerCase()}_txs_per_day,
    (${chains[chn].name.toLowerCase()}.tpb) AS ${chains[chn].name.toLowerCase()}_avg_tpb,
    (${chains[chn].name.toLowerCase()}.tps) AS ${chains[chn].name.toLowerCase()}_avg_tps,
    (${chains[chn].name.toLowerCase()}.tx_fees) AS ${chains[chn].name.toLowerCase()}_tx_fees,
    (${chains[chn].name.toLowerCase()}.avg_fee) AS ${chains[chn].name.toLowerCase()}_avg_fee,
    (${chains[chn].name.toLowerCase()}.users) AS ${chains[chn].name.toLowerCase()}_users_per_day`
    );
    ...
    const query = `SELECT 
    ${chains[selectedChains[0]].name}.date as date,
    ${columns.join(',')}
    FROM 
    ${tables}
    order by date`;
...
const blocktime_query = ({days,selectedChains}) => {
    ...
    const columns = selectedChains.map((chn) => 
    `(${chains[chn].name.toLowerCase()}.AVG_block_time_defference) as ${chains[chn].name.toLowerCase()}_avg_block_time,
    (${chains[chn].name.toLowerCase()}.Max_between_block_time) as ${chains[chn].name.toLowerCase()}_max_block_time,
    (${chains[chn].name.toLowerCase()}.Min_between_block_time) as ${chains[chn].name.toLowerCase()}_min_block_time`
    );
